...
public final class RectangleFloat implements Rectangle{
    ...
    public Rectangle add(Rectangle r) {
        if (r.isDoublePrecision()) {
            return RectangleDouble.create(Math.min(x1, r.x1()), Math.min(y1, r.y1()), Math.max(x2, r.x2()),
                    Math.max(y2, r.y2()));
        } else if (r instanceof RectangleFloat) {
            RectangleFloat rf = (RectangleFloat) r;
            return RectangleFloat.create(Math.min(x1, rf.x1), Math.min(y1, rf.y1), Math.max(x2, rf.x2),
                    Math.max(y2, rf.y2));
        } else {
            PointFloat rf = (PointFloat) r;
            return RectangleFloat.create(Math.min(x1, rf.xFloat()), Math.min(y1, rf.yFloat()),
                    Math.max(x2, rf.xFloat()), Math.max(y2, rf.yFloat()));
        }
    }
    ...
    public double intersectionArea(Rectangle r) {
        if (!intersects(r))
            ...
            return RectangleDouble
                    .create(Math.max(x1, r.x1()), Math.max(y1, r.y1()), Math.min(x2, r.x2()), Math.min(y2, r.y2()))
                    .area();
    }
